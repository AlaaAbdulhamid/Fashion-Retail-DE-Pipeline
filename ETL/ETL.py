...
# NOTE: This script was extracted from the exploratory notebook (FashionRetail.ipynb)
# as an early consolidation step. Paths and credentials are parameterized via
# environment variables below, falling back to the original local development values.
POSTGRES_PASSWORD = os.environ.get("POSTGRES_PASSWORD", "1699")
SPARK_JAR_PATH = os.environ.get("SPARK_JAR_PATH", "jars/postgresql-42.7.5.jar")
TRANSACTIONS_CSV_PATH = os.environ.get("TRANSACTIONS_CSV_PATH", "data/Transactions2.csv")
...
pandas_df.to_csv(f"{OUTPUT_DIR}/Date.csv", index=False)

# Save Customers DWH
pandas_df = customers_dwh.toPandas()
pandas_df.to_csv(f"{OUTPUT_DIR}/Customers.csv", index=False)

# Save Discounts DWH
pandas_df = discounts_dwh.toPandas()
pandas_df.to_csv(f"{OUTPUT_DIR}/Discounts.csv", index=False)

# Save Products DWH
pandas_df = products_dwh.toPandas()
pandas_df.to_csv(f"{OUTPUT_DIR}/Products.csv", index=False)

# Save Stores DWH
pandas_df = stores_dwh.toPandas()
pandas_df.to_csv(f"{OUTPUT_DIR}/Stores.csv", index=False)

# Save Employees DWH
pandas_df = employees_dwh.toPandas()
pandas_df.to_csv(f"{OUTPUT_DIR}/Employees.csv", index=False)

# Save Transactions Fact Table
pandas_df = transactions_dwh.toPandas()
pandas_df.to_csv(f"{OUTPUT_DIR}/Transactions1_2.csv", index=False)